...
\begin{enumerate}
  \item 
  $T'(u)$ and $Y(u)$ are independent. 
  \item
  $Y(u)$ has the same distribution as $J$ conditional on $J > u$. 
\end{enumerate}
\end{proposition}
...
\begin{align*}
\PP(J_1 - u > y | J_1 > u) 
\approx (1+ \xi y / \tilde \sigma)^{-1/\xi} \rightarrow 1 - H(y) \text{ as } u \uparrow x_R,
\end{align*}
where $\tilde \sigma :=\sigma + \xi(u-\mu)$, $y>0$ and $1+\xi y/\tilde \sigma >0$. A distribution with CDF $H(y)$ is said to be from the \textbf{Generalised Pareto family}
$GP(\xi,\bar \sigma)$.\\  
